class FoodRatings {
    Map<String, Integer> food_ratings = new HashMap<>();
    ...
    public FoodRatings(String[] foods, String[] cuisines, int[] ratings) {
        for(int i = 0; i < foods.length; i++){
            ...
            cuisine_ratings_food.computeIfAbsent(cuisines[i], k -> new TreeSet<>((a,b)   -> {
                int cmp = Integer.compare(a.getKey(), b.getKey());
                if(cmp == 0){
    ...
    public void changeRating(String food, int newRating) {
        String cuisineName = food_cuisine.get(food);
         ...
         Pair<Integer, String> oldElement = new Pair<>(-food_ratings.get(food), food);
